...
const Edit = () => {
  ...
  useEffect(()=>{console.log('atedit user',user)},[user])
...
const navigate=useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(Api.editUserInfo.url, formData, {
        headers: {
  ...
  return (
    <div className='max-w-sm mx-auto outline-1 outline-green-400 outline-dashed'>
      ...
      <form className='flex flex-col mt-3 gap-2' onSubmit={handleSubmit}>
        ...
        <button type='submit' className='bg-green-400 text-white p-3'>confirm</button>
      </form>
    </div>
  );
};
